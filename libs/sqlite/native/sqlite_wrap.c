...
static int sqlite_connection_resource_type;
static int sqlite_statement_resource_type;
...
static int column_index(CielSqliteStatement *statement, size_t index,
                        int *out) {
...
static int32_t sqlite_connection_resource_close(void *ptr) {
    return (int32_t)ciel_sqlite_close((CielSqliteConnection *)ptr);
}

static int32_t sqlite_statement_resource_close(void *ptr) {
    return (int32_t)ciel_sqlite_finalize((CielSqliteStatement *)ptr);
}

static int sqlite_handle_out(CielResourceHandle handle, uint64_t *out_owner,
                             uint64_t *out_resource, uint64_t *out_generation) {
    if (out_owner == NULL || out_resource == NULL || out_generation == NULL)
        return SQLITE_MISUSE;
    *out_owner = handle.owner_id;
    *out_resource = handle.resource_id;
    *out_generation = handle.generation;
    return SQLITE_OK;
}

static CielResourceHandle sqlite_handle(uint64_t owner, uint64_t resource,
                                        uint64_t generation) {
    CielResourceHandle handle;
    handle.owner_id = owner;
    handle.resource_id = resource;
    handle.generation = generation;
    return handle;
}

static int sqlite_register_connection(CielSqliteConnection *connection,
                                      uint64_t *out_owner,
                                      uint64_t *out_resource,
                                      uint64_t *out_generation) {
    CielResourceHandle handle;
    int32_t rc = ciel_resource_register_native(
        connection, sqlite_connection_resource_close,
        &sqlite_connection_resource_type, &handle);
    if (rc != 0)
        return rc;
    return sqlite_handle_out(handle, out_owner, out_resource, out_generation);
}

static int sqlite_register_statement(CielSqliteStatement *statement,
                                     uint64_t *out_owner,
                                     uint64_t *out_resource,
                                     uint64_t *out_generation) {
    CielResourceHandle handle;
    int32_t rc = ciel_resource_register_native(
        statement, sqlite_statement_resource_close,
        &sqlite_statement_resource_type, &handle);
    if (rc != 0)
        return rc;
    return sqlite_handle_out(handle, out_owner, out_resource, out_generation);
}
...
int ciel_sqlite_open(const char *path, size_t path_len, int flags,
    ...
    int rc = wrap_open_cstr(c_path, sqlite_open_flags(flags), &raw);
    if (rc != SQLITE_OK)
        return rc;
    rc = sqlite_register_connection(raw, out_owner, out_resource,
                                    out_generation);
    if (rc != SQLITE_OK)
        (void)ciel_sqlite_close(raw);
    return rc;
}
...
int ciel_sqlite_open_memory(uint64_t *out_owner, uint64_t *out_resource,
                            uint64_t *out_generation) {
    CielSqliteConnection *raw = NULL;
    int rc = wrap_open_cstr(
        ":memory:",
        SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_MEMORY, &raw);
    if (rc != SQLITE_OK)
        return rc;
    rc = sqlite_register_connection(raw, out_owner, out_resource,
                                    out_generation);
...
int ciel_sqlite_close_handle(uint64_t owner, uint64_t resource,
                             uint64_t generation) {
    return ciel_resource_close(sqlite_handle(owner, resource, generation));
}
...
int ciel_sqlite_connection_snapshot(uint64_t owner, uint64_t resource,
    ...
    if (out == NULL)
        return SQLITE_MISUSE;
    void *raw = NULL;
    int32_t rc = ciel_resource_native_snapshot(
        sqlite_handle(owner, resource, generation),
        &sqlite_connection_resource_type, &raw);
    if (rc != 0)
        return rc;
    *out = (CielSqliteConnection *)raw;
    return SQLITE_OK;
}
...
int ciel_sqlite_prepare(CielSqliteConnection *connection, const char *sql,
    ...
    rc = sqlite_register_statement(statement, out_owner, out_resource,
                                   out_generation);
    if (rc != SQLITE_OK)
        (void)ciel_sqlite_finalize(statement);
    return rc;
}
...
int ciel_sqlite_statement_snapshot(uint64_t owner, uint64_t resource,
    ...
    int32_t rc = ciel_resource_native_snapshot(
        sqlite_handle(owner, resource, generation),
        &sqlite_statement_resource_type, &raw);
    if (rc != 0)
        return rc;
    *out = (CielSqliteStatement *)raw;
    return SQLITE_OK;
}
...
int ciel_sqlite_bind_text(CielSqliteStatement *statement, size_t index,
    ...
    return sqlite3_bind_text64(
        statement->stmt, sqlite_index, text == NULL ? "" : text,
        (sqlite3_uint64)text_len, SQLITE_TRANSIENT, SQLITE_UTF8);
}
